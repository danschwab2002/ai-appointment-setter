...
await db.exec(`
  insert into public.followup_policy_versions (
    ...
    'precheckout-worker-test', 1, 'published', 'cart_recovery', 'UTC',
  ...
  insert into public.hotmart_abandonment_timer_policy_bindings (
    ...
    true, 'precheckout-worker-test', 1
  );
  insert into public.pilot_scope_versions (
    ...
    'cart_recovery', 'precheckout-worker-test', 1, 'UTC',
    1, 1, 1, 'fixture-only', now(), now()
  );
  insert into public.pilot_runtime_controls (
    ...
    'fixture-only', 'Worker/sender tracer; no HTTP adapter is present.'
  );
`);
